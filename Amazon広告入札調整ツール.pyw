...
class AmazonBidAdjusterApp:
    ...
    def run_sp_sb_adjustment(self, col, search_col, ad_type):
        ...
        new_kw_candidates, skipped_existing = self._find_new_keywords(search_term_data, target_acos_map, original_bid_map, p, existing_keywords)
    ...
    def _find_new_keywords(self, data, target_acos_map, original_bid_map, p, existing_keywords):
        portfolio_terms = {}
        for item in data:
        ...
        for (term, portfolio), items in portfolio_terms.items():
            ...
            if (term_normalized, portfolio) in existing_keywords:
                skipped_existing += 1
                continue  # 既存のため除外
